...
export function validateDayItem(x) {
  ...
  return Object.freeze(Object.assign({}, x, { title: x.title.trim() }));
}
...
export async function createDayItem(input) {
  ...
  const item = validateDayItem({ id: 'day-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2,9), title: input.title, dateKey: input.dateKey, time: input.time || '', type: input.type || 'task', status: 'open', createdAt: now, updatedAt: now });
