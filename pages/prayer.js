...
function mountQiblaScreen(page, appContext) {
  ...
  var handler = null;
  function start() { enable.hidden = true; text.textContent = 'Lëvize telefonin ngadalë për kalibrim.'; handler = function (event) { var pointer = needle.querySelector('[data-qibla-pointer]'); var beta = Number(event.beta); var gamma = Number(event.gamma); var horizontal = Number.isFinite(beta) && Number.isFinite(gamma) && Math.abs(beta) <= 25 && Math.abs(gamma) <= 25; if (!horizontal) { dial.classList.add('qibla-dial--tilted'); text.textContent = 'Mbaje telefonin sa më horizontal për një drejtim më të saktë.'; return; } dial.classList.remove('qibla-dial--tilted'); var heading = Number.isFinite(event.webkitCompassHeading) ? event.webkitCompassHeading : (Number.isFinite(event.alpha) ? (360 - event.alpha) % 360 : null); if (!Number.isFinite(heading)) { text.textContent = 'Kompasi nuk po jep drejtim. Lëvize telefonin ngadalë në formë 8 për kalibrim.'; return; } var turn = (bearing - heading + 360) % 360; if (pointer) pointer.setAttribute('transform', 'rotate(' + turn + ')'); text.textContent = 'Pozicion i mirë. Rrotullo telefonin derisa treguesi të jetë lart.'; }; window.addEventListener('deviceorientation', handler); }
